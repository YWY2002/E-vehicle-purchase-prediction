...
op_1_output = s3_source(spark=spark, mode=mode, **{'dataset_definition': {'datasetSourceType': 'Local File Upload', 'name': 'train.csv', 'description': None, 's3ExecutionContext': {'s3Uri': 's3://sagemaker-ap-southeast-1-737069837987/Canvas/default-20260904T230821/uploads/train.csv', 's3ContentType': 'csv', 's3HasHeader': True, 's3FieldDelimiter': ',', 's3DirIncludesNested': False, 's3AddsFilenameColumn': False, 's3RoleArn': None, 's3CsvEncodingType': 'UTF_8', 's3SkipLines': 0, 's3MultiLine': False, 's3DataType': 'S3Prefix', 's3ManifestPlain': {'s3Uris': None}}, 'canvasDatasetMetadata': None}})
op_2_output = infer_and_cast_type(op_1_output['default'], spark=spark, **{})
op_3_output = encode_categorical(op_2_output['default'], spark=spark, **{'operator': 'One-hot encode', 'one_hot_encode_parameters': {'input_column': ['City_Type', 'Current_Car_Type', 'Gender'], 'invalid_handling_strategy': 'Keep', 'drop_last': False, 'output_style': 'Columns'}, 'ordinal_encode_parameters': {'invalid_handling_strategy': 'Replace with NaN'}})
op_4_output = encode_categorical(op_3_output['default'], spark=spark, **{'operator': 'Ordinal encode', 'ordinal_encode_parameters': {'input_column': ['Will_Buy_EV', 'Range_Anxiety_Level', 'Home_Charging_Possible', 'Subsidy_Available'], 'invalid_handling_strategy': 'Skip'}})

#  Glossary: variable name to node_id
#
#  op_1_output: 95747ed4-9767-4432-997d-663f23669d78
#  op_2_output: f66ff107-2495-43ad-910e-5af2f26db8d0
#  op_3_output: 449a2647-131d-45a6-b7c6-2c2d7750b561
#  op_4_output: 063f15d5-364b-42ae-98f0-58cdaba8b428